import React from 'react';
import "../section2/Section2.modul.css"

const Section2 = () => {
    return (
        <div className='section2'>
            <div className='section2-header'>
                <h2>Наши поставщики товаров </h2>
            </div>
            <div className='section2-item'>
                <img className='section2-image' src="https://i01.appmifile.com/webfile/globalimg/mobile/2015/milogo.png" alt="" />
                <img className='section2-image' src="https://logos-world.net/wp-content/uploads/2020/04/Apple-Logo-1998-present.png" alt="" />
                <img className='section2-image' src="https://pngimg.com/uploads/samsung_logo/samsung_logo_PNG5.png" alt="" />
                <img className='section2-image' src="https://seeklogo.com/images/H/huawei-logo-6535FE0552-seeklogo.com.png" alt="" />
                <img className='section2-image' src="https://автопрайс.рф/wp-content/uploads/2020/03/logo-3.jpg" alt="" />
                
                
            </div>

            
        </div>
    );
};

export default Section2;