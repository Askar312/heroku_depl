...
export default function Cart() {
  ...
  return (
    <TableContainer sx={{marginBottom:47}} component={Paper}>
        ...
        <TableBody>
          {cart.products.map((row) => (
            <StyledTableRow key={row.item.name}>
              ...
              <StyledTableCell align="right">
                <Button sx={{color:"red",border:1}} onClick={() => deleteCartProducts(row.item.id)}>
                  Удалить
                </Button>
              </StyledTableCell>
            </StyledTableRow>
      ...
      <Button sx={{ color:"red",border:1,my:1,mx:1}} onClick={cartCleaner}>Купить за {cart.totalPrice} сом</Button>
    </TableContainer>
  );
}
